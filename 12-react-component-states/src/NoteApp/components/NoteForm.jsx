...
function NoteForm({
  ...
  const handleUpdateFormData = (e) => {
    ...
    // title, content, userID에 해당하는 프로퍼티와 input or selector 값을 키: 값으로 설정해서
    const nextFormData = {
      ...formData,
      [name]: value,
    };

    // 새롭게 생성된 form data로 state update
  ...
  const handleCreateNote = (e) => {
    ...
    const newNote = {
      id: newNoteId,
      title: title.trim(),
      // text를 HTML 태그로 변경하는 함수 실행
      content: convertTextToHTMLString(content),
      userId: newUserId,
      // 새롭게 생성된 userID로 user data 가져와서 확장
      expand: {
  ...
  const handleEditNote = (e) => {
    ...
    const willEditNote = {
      ...note, // 기존 노트의 모든 속성을 복사
      ...formData, // 폼에서 수정된 속성을 덮어쓰기
    };
  ...
  // 노트 초기화 기능(create 모드) => 아직 버튼만 만들고 기능 구현 X
  const handleReset = (e) => {
    e.preventDefault();
  };
  ...
  return (
    <form className="NoteForm" onSubmit={handleSubmit} onReset={handleReset}>
      ...
      <div className="buttonGroup">
        ...
        {/* mode=create면 초기화 버튼 렌더링, edit이면 삭제 버튼으로 렌더링 */}
        {isCreateMode ? (
